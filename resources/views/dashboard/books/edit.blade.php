...
<form action="/dashboard/books/{{ $book->slug }}" method="POST" enctype="multipart/form-data">
    @method('put')
    @csrf
  <div class="text-2xl font-bold mb-4">Edit Buku</div>
  <div class="grid gap-6 mb-6 md:grid-cols-1">
      <div>
          <label for="title" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Judul Buku</label>
          <input type="text" id="title" name="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Judul Buku" required value="{{ old('title', $book->title) }}">
          @error('title')
      ...
      <div>
        ...
        <input type="text" id="stock_now" name="stock_now" data-input-counter aria-describedby="helper-text-explanation" class="bg-gray-50 border-x-0 border-gray-300 h-11 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="0" required value="{{ old('stock_now', $book->stock_now) }}">
      ...
      <div>
        ...
        <img src="{{ asset('/storage/'.$book->image) }}" alt="" id="imagePreview" class="w-20 self-center">
        @error('image')
          <p class="text-sm text-red-600 underline">{{ $message }}</p>
          @enderror
      </div>
